🔄 **Live page** — refreshed daily during the on-call week (2026-09-08 → 2026-09-15). Last refreshed **2026-09-13 10:00 AM PT (America/Los_Angeles)** (\~5 days / 120 h into the week). This page freezes at the Tuesday handoff (2026-09-15 11:00 America/Mexico_City); a new page opens for the next week.

🔴 **Carry-in from the week just closed — watch these.** (1) **First-cashout volume drop** [17131362](https://app.datadoghq.com/monitors/17131362) — the Aug 7 \~15:00 UTC cliff persists (\~37 days); reads OK only because the anomaly model adapted (quiet ≠ recovered). Real signal — do NOT tune → Activation runbook `kem-tug-987` + Jira. (2) **Funnel-cashout expirations low** [143509449](https://app.datadoghq.com/monitors/143509449) — fired 2× this week (High, acked) at the tightened `< 2` threshold; the Sep 10 fire is still open as a stale incident.io alert (Datadog No Data). The real drop persists → Jira / throughput fix, do NOT tune further. Prior week (Sep 1 → Sep 8) closed at **34 records** (14 High / 20 Low), all resolved, 0 incidents.

# Growth Team Ops Review — Weekly Handoff

**09/13/2026 Growth Team Ops Review** · On-call week **2026-09-08 11:00 → 2026-09-15 11:00** (America/Mexico_City) · Sources: incident.io + Datadog (read-only) + Jira (vulnerabilities) · Last refreshed: **2026-09-13 10:00 AM PT** (week-to-date, \~5 days in; live, refreshed daily).
...
**Alert volume — week-to-date (\~5 days / 120 h in):** **19 records** (9 High, 10 Low) — 18 resolved, 1 still firing (a stale funnel-cashout alert, Datadog No Data). Most recent fires Sep 12 \~11:47 AM PT (svc-notification-preferences latency pair); \~22 h quiet since. | **Prior full week (Sep 1 → Sep 8):** 34 records (14 High, 20 Low), all resolved, 0 incidents. | **Trend: ↓ (lighter)** — 19 records in the first \~5 days vs **25** in the prior week's same 5-day slice; run-rate \~27/wk (the prior full week's 34 was front-loaded — all 25 of the same-slice landed by day 3 — so the same-slice comparison is the fair read). **Human-attention: 12 | Auto-resolved: 7.** **Escalation rate (alerts → incidents): 0/19 (0%).** **Still firing: 1 active / 5 stale** (incident.io) — one Growth monitor now reads Alert (the Segment-API-calls anomaly [243691582](https://app.datadoghq.com/monitors/243691582), Slack-only / non-paging, so absent from the 19 records); the 5 stale are 4 old orphans plus the Sep 10 lingering funnel-cashout alert (below).
...
| --- | --- | --- | --- | --- |
| [301972958](https://app.datadoghq.com/monitors/301972958) — svc-mark-tech Request duration too high | <custom data-type="status" data-id="id-0">High</custom> | svc-mark-tech | Edder Núñez | **TL;DR:** svc-mark-tech request-duration (p99) crossed its 10 s page 3× on Sep 8 evening PT (Datadog P5 events routed to incident.io High); Edder acked each in \~6–16 s and all self-resolved in \~4–10 min — no customer impact. An engineer widened the monitor's window on Sep 9 and it has been quiet since. **What happened:** Observed — monitor 301972958 (now `percentile(last_1d):p99:trace.fastapi.request{service:svc-mark-tech} by {kube_cluster_name,env} > 10`, prod branch routes @webhook-incidentio-high) fired 3×: (1) 2026-09-09 03:15 UTC (\~8:15 PM PT Sep 8), acked \~15 s, resolved 03:19 UTC; (2) 2026-09-09 03:20 UTC (\~8:20 PM PT Sep 8), acked \~6 s, resolved 03:26 UTC; (3) 2026-09-09 04:14 UTC (\~9:14 PM PT Sep 8), acked \~16 s, resolved 04:24 UTC. Env prod (alert scope `env:prod,kube_cluster_name:production-eks-cluster`); monitor now OK. **Config change observed** (Datadog monitor audit, `source:monitor_audit_event`): on 2026-09-09 the evaluation window was widened `last_10m → last_4h → last_1d` (04:33 and 09:08 UTC) — all 3 fires predate it and there have been 0 fires since. That is effectively the "require sustained" half of the standing rec, but a `last_1d` p99 is very smooth and may miss a real sub-day latency regression (p99 averages \~5.7 s over 14 d, so `> 10 s` is \~1.75× normal); the prod branch still pages High and carries a Datadog `[P5]` tag (the P5→High over-route theme). Prefer `last_1h` + route brief blips to Low. Cause not determined from available signals (brief p99 request-duration spikes that self-cleared). |
...
| [243692163](https://app.datadoghq.com/monitors/243692163) — svc-notification-preferences high p90 latency | <custom data-type="status" data-id="id-3">High</custom> | svc-notification-preferences | Edder Núñez | **TL;DR:** svc-notification-preferences p90 latency crossed its `> 1 s` page 4× (Sep 9 night PT through Sep 12 midday PT); Edder acked three in seconds and the fourth — a \~1:36 AM PT Sep 9 night page — self-resolved with no ack; all cleared in \~9–15 min, no customer impact. **What happened:** Observed — monitor 243692163 (`avg(last_10m):p90:trace.aspnet_core.request{env:prod,service:svc-notification-preferences} > 1`, routes @webhook-incidentio-high) fired 4×: (1) 2026-09-09 08:36 UTC (\~1:36 AM PT Sep 9), escalation cancelled (no ack), self-resolved 08:46 UTC (\~10 min) — a High-routed page in sleeping hours nobody had to act on; (2) 2026-09-10 23:41 UTC (\~4:41 PM PT Sep 10), acked 23:41:18 (\~13 s), resolved 23:56 UTC; (3) 2026-09-11 04:41 UTC (\~9:41 PM PT Sep 10), acked 04:41:14 (\~9 s), resolved 04:50 UTC; (4) 2026-09-12 18:47 UTC (\~11:47 AM PT Sep 12), acked 18:47:18 (\~14 s), resolved 19:00 UTC. Monitor now OK; the query has no sustain (single 10-min window). At 4× it stays above the ≥ 3-fires noise bar. Likely cause: brief p90 latency blips (p90 baseline \~0.30 s / 14 d avg, so `> 1 s` is \~3× normal) — a sustain guard would suppress this class. |
...
| [301972958](https://app.datadoghq.com/monitors/301972958) — svc-mark-tech Request duration too high | 3× (all Sep 8 evening PT) | P5→High over-route: fired 3× overnight, all fast-acked and self-resolved in minutes. **Observed config change Sep 9** — eval window widened `last_10m → last_1d` (a partial sustain); 0 fires since, but `last_1d` p99 may mask a real sub-day regression. Still route the prod branch to Low or require ≥ N consecutive windows, and prefer `last_1h`. Ledger weeks_seen 7 (→ 8 at the Sep 15 roll); see Action Items. |
| [243692163](https://app.datadoghq.com/monitors/243692163) — svc-notification-preferences high p90 latency | 4× (Sep 9–12) | Above the ≥ 3-fires bar (3 acked + a \~1:36 AM PT night page, cancelled). Add a sustain / duration guard (keep the `> 1 s` bound; p90 baseline \~0.30 s / 14 d avg). weeks_seen 4; see Action Items. |
...
_Top 5 by expected impact; **full history (25 rows) →**_ [_Monitor Tuning Ledger_](https://earnin.atlassian.net/wiki/spaces/~712020cb7ebe6a714e411e98574e2fb19d5faa/pages/5322604577)_. Standing, learned recommendations carried into the week — they persist until applied. This week's noisiest were the_ **HPA flapper** _(_`135119948` _×5) and the_ **svc-notif p90** _(_`243692163` _×4); the_ **svc-mark-tech P5 → High over-route** _(_`301972958` _×3) had its eval window widened Sep 9 (see its finding). See Action Items._

### 🔴 Open Going Into the Week

**Active — Datadog Alert/Warn now: 1.** Monitor [243691582](https://app.datadoghq.com/monitors/243691582) — "Anomaly in calls to Segment API" (svc-notification-preferences) currently reads **Alert**: a robust weekly-anomaly on Segment `ProcessEvent` timing (`cluster_flavor:prod, destination:_segment`), routed Slack-only (`@slack-growth-engineering-alerts`), so it did not page and is absent from the 19 incident.io records. Newly Alert since the Sep 12 refresh (was 0 active). Investigate whether it reflects a real Segment API processing anomaly (may affect notification-preferences toggle set/get) or anomaly-model noise; cause not determined from available signals. Do not tune before confirming.

**Stale / lingering incident.io alerts (need a manual clear, not active prod work): 5** (all confirmed still-firing via `alert_list status:firing` and absent from the current Datadog Alert/Warn set):
...
- [ ] **Escalate the first-cashout volume drop** (\~37 days below baseline) via the Activation runbook / dashboard kem-tug-987 ([17131362](https://app.datadoghq.com/monitors/17131362)); check the Aug 7 \~15:00 UTC cliff (deploy vs demand); open a Jira fix. Do NOT tune the monitor.
...
- [ ] **Investigate the Segment API call anomaly** ([243691582](https://app.datadoghq.com/monitors/243691582), active/Alert now, Slack-only): confirm whether Segment `ProcessEvent` timing is genuinely anomalous (may affect notification-preferences toggles) or anomaly-model noise; do not tune before confirming.
...
- [ ] **Add a sustain to the svc-notification-preferences latency pair** ([243692163](https://app.datadoghq.com/monitors/243692163) p90 High / [243692043](https://app.datadoghq.com/monitors/243692043) avg Low): p90 fired 4× this week (above the ≥ 3-fires bar; incl. a \~1:36 AM PT night page); keep the bounds (p90 baseline \~0.30 s / 14 d avg), require ≥ 2 consecutive windows; consider consolidating the two.
...
_Generated by the Growth Team Ops Review agent. Window: 2026-09-08 11:00 → 2026-09-15 11:00 America/Mexico_City (week-to-date, \~5 days in; live, refreshed daily until it freezes at the Sep 15 handoff). Last refreshed: 2026-09-13 10:00 AM PT. Sources: incident.io (read-only) + Datadog (read-only) + Jira (vulnerabilities) — all connectors healthy. Customer identifiers redacted where present (none required this run). You can read #growth-engineering-alerts for more information. No monitoring configuration was changed by this agent._