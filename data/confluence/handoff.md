🔄 **Live page** — refreshed daily during the on-call week (2026-09-22 → 2026-09-29). Last refreshed **2026-09-24 10:07 AM PT (America/Los\_Angeles)** (day 3, \~2 days into the week). This page freezes at the Tuesday handoff (2026-09-29 11:00 America/Mexico\_City); a new page opens for the next week.

🌟 **On-call week, day 3 (\~2 days in).** **1 incident.io Growth alert record** (a Low, auto-resolved) and **0 incidents** since the Sep 22 handoff. Datadog logged **69 monitor fire transitions** (112 events incl. recoveries) across 6 monitors — the standing SQS-backlog trio (first-mile / deactivate-user / user-activation), two OTGE svc-earnings monitors (CPU-throttle [111957818](https://app.datadoghq.com/monitors/111957818), HPA [111957822](https://app.datadoghq.com/monitors/111957822)), and a single [119674469](https://app.datadoghq.com/monitors/119674469) postman latency Warn — each self-recovered in minutes. Only the postman Low routed to incident.io (auto-resolved); the other 5 were **0-page flappers**; **0 monitors read Alert/Warn now**. Stale incident.io alerts **hold at 5** (re-verified via `alert_stats` — the `alert_list` attribute filter under-returned 1 this run; per-alert `alert_show` confirms all 5 still firing). incident.io authenticated and healthy; Datadog + Jira healthy. Vulnerabilities **36 → 45** (org-wide, volatile).

# Growth Team Ops Review — Weekly Handoff

**09/24/2026 Growth Team Ops Review** · On-call week **2026-09-22 11:00 → 2026-09-29 11:00** (America/Mexico\_City) · Sources: incident.io + Datadog (read-only) + Jira (vulnerabilities) · Last refreshed: **2026-09-24 10:07 AM PT** (week-to-date, day 3; live, refreshed daily).
...
*Coverage check (Slack out-of-office, as of 2026-09-24 10:07 AM PT): could not be completed (no Slack profile-read tool available under either name) — verify availability manually. incident.io *`schedule_show`* shows no PTO booked for the rotation members (Ankur Shivani, Alfred, aiden.ramgoolam) this window.*
...
**Alert volume — week-to-date (day 3, \~2 days in):** **1 incident.io Growth record** (0 High, 1 Low), auto-resolved. Datadog: **69 monitor fire transitions** across 6 monitors — first-mile SQS [137629294](https://app.datadoghq.com/monitors/137629294) ×40, deactivate-user SQS [137629364](https://app.datadoghq.com/monitors/137629364) ×14, user-activation SQS [137629650](https://app.datadoghq.com/monitors/137629650) ×11, OTGE CPU-throttle [111957818](https://app.datadoghq.com/monitors/111957818) ×2, OTGE HPA [111957822](https://app.datadoghq.com/monitors/111957822) ×1, postman latency [119674469](https://app.datadoghq.com/monitors/119674469) ×1 — all self-recovered in minutes; only the postman Low created an incident.io record. | **Prior full week (Sep 15 → Sep 22, closed):** 2 incident.io records (1 High + 1 Low), both auto-resolved, 0 incidents; \~130 Datadog transitions. | **Trend:** incident.io run-rate \~3/wk vs prior 2 → nominally ↑ (a single Low auto-resolve — effectively flat; nothing High paged). Datadog flapper run-rate \~240/wk (elevated vs prior \~130) — standing SQS noise, 0 pages. **Human-attention: 0 · Auto-resolved: 1 · Escalation rate (alerts → incidents): 0/1 (0%). Still firing: 0 active / 5 stale** (incident.io, re-verified via `alert_stats` this run).
...
**No operational incidents this week** (incident.io → 0). **No monitor config changes** since the Sep 22 handoff (Datadog `monitor_audit_event`, `team:l2-peng-growth` → 0). The funnel monitor [143509449](https://app.datadoghq.com/monitors/143509449) was deleted Sep 22 15:43 UTC — that was before this handoff (prior window), so it is not a change in this week's window.
...
- **TL;DR:** [119674469](https://app.datadoghq.com/monitors/119674469) — `service-postman-internal` gRPC average latency briefly crossed the Low Warn threshold on **prod** overnight; routed low-urgency, no human ack, self-resolved in \~8 min — no customer impact.   
...
| --- | --- | --- |
| First Mile Calc SQS backlog — [137629294](https://app.datadoghq.com/monitors/137629294) | 40 (Warn) | Standing `last_5m` oldest-age flapper (SQS `first-mile-new-user-score`); each self-cleared \~1–3 min; 0 incident.io pages. Tuning: add `env:prod` + a 10–15 min sustain. |
| Deactivated-User SQS backlog — [137629364](https://app.datadoghq.com/monitors/137629364) | 14 (Warn \> 60 + Alert \> 90) | Same SQS cluster (`deactivate-user`); crossed Alert a few times but self-cleared in minutes; 0 pages. |
| User-Activation SQS backlog — [137629650](https://app.datadoghq.com/monitors/137629650) | 11 (Warn \> 120 + Alert \> 150) | Same SQS cluster (`prod_statusactivationattempt`); self-cleared \~1 min; 0 pages. |
| OTGE CPU throttling — [111957818](https://app.datadoghq.com/monitors/111957818) | 2 (Alert) | svc-earnings OTGE, `env:prod`, CPU CFS throttled ≥ 1 (last\_10m); self-recovered \~1 min; Service-Advisor monitor, no notification handles → 0 page. Infra saturation autoscaling/limits handles. |
| OTGE HPA sustained high util — [111957822](https://app.datadoghq.com/monitors/111957822) | 1 (Warn) | New this week: svc-earnings OTGE HPA on `production-eks-cluster` (\~82% of max replicas \> 90 sustained); Service-Advisor, no handles → 0 page; self-recovered \~6 min. Same infra-saturation pattern as HPA [135119948](https://app.datadoghq.com/monitors/135119948) — below the noise bar (1 fire); watch. |

*Every Datadog fire this week is standing SQS-backlog + OTGE flapper noise (0 incident.io pages); the one postman Low is in Auto-Resolved above. Full standing-candidate history → the *[Monitor Tuning Ledger](https://earnin.atlassian.net/wiki/spaces/~712020cb7ebe6a714e411e98574e2fb19d5faa/pages/5322604577/Growth+Team+Ops+Review+Monitor+Tuning+Ledger)*.*
...
| [137629294](https://app.datadoghq.com/monitors/137629294) / [137629364](https://app.datadoghq.com/monitors/137629364) / [137629650](https://app.datadoghq.com/monitors/137629650) — SQS backlog cluster | Flappy backlog, no env scope / no sustain (Datadog noise; 0 incident.io pages) | Fired 40 / 14 / 11 in \~2 days this week (65 fires; run-rate well above last week); weeks\_seen 10 / 9 / 9; 0 pages | **Add scope + sustain; verify routing.** before: SQS oldest-age Alert on `last_5m`, no env scope, no sustain. after: add `env:prod` + a sustain ≥ 10–15 min; verify the prod routing branch resolves. Coverage: a sustained real backlog still alerts. | med | RECOMMEND |
...
*Top 5 by expected impact; **full history (30 rows) →*** [Monitor Tuning Ledger](https://earnin.atlassian.net/wiki/spaces/~712020cb7ebe6a714e411e98574e2fb19d5faa/pages/5322604577/Growth+Team+Ops+Review+Monitor+Tuning+Ledger)*. Carried across the week; weeks\_seen holds until the next Tue handoff (2026-09-29).*

### 🔴 Open Going Into the Week

**Active — Datadog Alert/Warn now: 0.** No Growth monitor reads Alert/Warn (`search_datadog_monitors team:l2-peng-growth status:(alert OR warn)` → none). The SQS/OTGE flapper crossings and the postman Low all self-recovered.

**Stale / lingering incident.io alerts: 5** (re-verified this run via `alert_stats` — 4 High + 1 Low; the `alert_list` attribute filter under-returned 1, but `alert_show` confirms all 5 still firing). None is an active prod problem; each needs a manual clear:
...
**Vulnerabilities:** **45 open** via [filter 15295 / OOSLA](https://earnin.atlassian.net/issues/?filter=15295) (org-wide, as of 2026-09-24 10:07 AM PT) — 32 CRITICAL / 12 HIGH by ticket summary prefix, plus 1 security ticket with no severity prefix (CORS misconfig `WEBPLAT-1489`, In Review). The 32 Criticals: 26 secrets-detection findings, `io.netty:netty-handler` (5), `System.Text.Encodings.Web` (1). The 12 High: `org.springframework:spring-core` (KMONO-62) plus new SCA dependency updates (browserslist ×4, fast-uri ×2, jwt, engine.io, google.golang.org, labstack/echo) and 1 secrets-detection High (EBBUD-3796). Severity from the ticket summary prefix (Jira priority field uniformly "Low"). Secrets tickets carry generic titles (no values). **org-wide** scope (no Growth-owned ticket). **Up +9 vs Sep 23 (36)** — High jumped 2 → 12 (mostly new SCA dep-update tickets); the Next.js (3) and bouncycastle (2) Criticals and the log-leak SAST High (SV-4848) dropped off. *Count is volatile intraday.*
...
* [ ] **Escalate the first-cashout volume drop** (\~48 days below baseline) via the Activation runbook / dashboard kem-tug-987 ([17131362](https://app.datadoghq.com/monitors/17131362)); check the Aug 7 \~15:00 UTC cliff (deploy vs demand); open a Jira fix. Do NOT tune the monitor.
...
* [ ] **Tune the SQS backlog cluster** ([137629294](https://app.datadoghq.com/monitors/137629294) / [137629364](https://app.datadoghq.com/monitors/137629364) / [137629650](https://app.datadoghq.com/monitors/137629650), \~65 fires in \~2 days this week): add `env:prod` + a 10–15 min sustain; verify the prod routing branch resolves.
* [ ] **Tune HPA **[135119948](https://app.datadoghq.com/monitors/135119948): route HIGH → LOW (or gate to critical-only); keep OOM / pod-not-ready at HIGH. Chronic (weeks\_seen 12). Watch the OTGE HPA [111957822](https://app.datadoghq.com/monitors/111957822) (same pattern, 1 fire this week — below the noise bar).
...
* [ ] Review open vulnerability tickets — 45 open (32 Critical / 12 High + 1 unlabeled), org-wide; 26 secrets-detection + 5 netty-handler Criticals lead, High jumped to 12 on new SCA dep-updates (+9 vs Sep 23).
...
*Generated by the Growth Team Ops Review agent. Window: 2026-09-22 11:00 → 2026-09-29 11:00 America/Mexico\_City (week-to-date; live, refreshed daily until it freezes at the Sep 29 handoff). Last refreshed: 2026-09-24 10:07 AM PT. Sources: incident.io (read-only) + Datadog (read-only) + Jira (vulnerabilities). Customer identifiers redacted where present (none required this run). You can read #growth-engineering-alerts for more information. No monitoring configuration was changed by this agent.*
